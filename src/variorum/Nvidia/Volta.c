...
int volta_get_power(int long_ver)
{
    ...
    {
        nvidia_dump_power_data(iter, long_ver, stdout);
    }
    return 0;
}
...
int volta_get_thermals(int long_ver)
{
    ...
    for (iter = 0; iter < nsockets; iter++)
    {
        nvidia_dump_thermal_data(iter, long_ver, stdout);
    }
    return 0;
}
...
int volta_get_clocks(int long_ver)
{
    ...
    for (iter = 0; iter < nsockets; iter++)
    {
        nvidia_dump_clocks_data(iter, long_ver, stdout);
    }
    return 0;
}
...
int volta_get_power_limits(int long_ver)
{
    ...
    for (iter = 0; iter < nsockets; iter++)
    {
        nvidia_dump_power_limits(iter, long_ver, stdout);
    }
    return 0;
}
